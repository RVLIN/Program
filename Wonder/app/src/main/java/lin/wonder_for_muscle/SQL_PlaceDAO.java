...
public class SQL_PlaceDAO {
    ...
    // 建構子，一般的應用都不需要修改
    public SQL_PlaceDAO(Context context) {
        db = SQL_MyDBHelper.getDatabase(context);
    }
    ...
    // 新增參數指定的物件
    public static SQL_Place insert(SQL_Place place) {
    ...
    // 修改參數指定的物件
    public boolean update(SQL_Place place) {
    ...
    // 取得指定編號的資料物件
    public SQL_Place get(long id) {
        // 準備回傳結果用的物件
        SQL_Place place = null;
        // 使用編號為查詢條件
        String where = KEY_ID + "=" + id;
        // 執行查詢
    ...
    // 把Cursor目前的資料包裝為物件
    public SQL_Place getRecord(Cursor cursor) {
        // 準備回傳結果用的物件
        SQL_Place result = new SQL_Place();
    ...
    public static void sampleData(Context context) {
        ...
        if (firstTime) {
            SQL_Place p01 = new SQL_Place(0, 25.04719, 121.516981, 10.0, "2011-12-31 08:30", "Hello!");
            SQL_Place p02 = new SQL_Place(0, 24.143033, 121.271982, 25.0, "2012-01-01 06:12", "Hi!");
            SQL_Place p03 = new SQL_Place(0, 25.200854, 121.646714, 55.0, "2012-02-12 16:50", "Awesome!");
            ...
            for (int i = 0; i < 10; i++) {
                ...
                SQL_Place place = new SQL_Place(0, 25.04719 + i, 121.516981 + i, i * 10, sdf.format(date), "Place: " + i);
                insert(place);
            }
